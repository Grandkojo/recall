...
description: A calm, white-canvas interface for Recall — a dementia care life memory app. The system anchors on a near-white canvas with the faintest blue tint, serif display headlines for warmth and storytelling, and a soft sky-blue primary that is easy on ageing eyes. Brand voltage comes from the white/blue pairing — clean and trustworthy without feeling clinical. Type voice runs a slab-serif display (Copernicus / Tiempos Headline) for h1/h2 and a humanist sans for body.
...
colors:
  ...
  muted-soft: "#8AA8C4"
  hairline: "#C5DDEF"
  hairline-soft: "#D5E8F5"
  canvas: "#F8FBFF"
  surface-soft: "#EDF4FC"
  surface-card: "#E2EFFA"
  surface-blue-strong: "#CCDFF5"
  surface-dark: "#0A1628"
  surface-dark-elevated: "#12233B"
  surface-dark-soft: "#0E1E32"
  on-primary: "#ffffff"
  on-dark: "#F0F7FF"
  on-dark-soft: "#8AA8C4"
  accent-teal: "#2BBFCF"
...
Recall is a calm, accessible memory interface for dementia care. The base atmosphere is a **near-white blue-tinted canvas** (`{colors.canvas}` — #F8FBFF) — fresh and clean without the sterile coldness of pure white or the clinical feel of grey. Headlines run a **slab-serif display** ("Copernicus" / Tiempos Headline) at weight 400 with negative letter-spacing, paired with **StyreneB / Inter** body sans. The combination feels warm and personal — like a family scrapbook, not a hospital app.

Brand voltage comes from the **white + soft blue pairing** — sky blue (`{colors.primary}` — #2D7DD2) is the Recall accent, used on every primary CTA and interactive element. The blue is calm and soft — easy on ageing eyes, trustworthy, never harsh or saturated.

The system has three surface modes:
1. **White-blue canvas** (`{colors.canvas}`) — default body floor
2. **Light blue cards** (`{colors.surface-card}`) — memory cards, feature card backgrounds
3. **Midnight blue product surfaces** (`{colors.surface-dark}`) — high-contrast patient-facing screens, footer

The dark surfaces serve the patient interface — where maximum contrast matters most for accessibility. The canvas-to-dark contrast is the system's pacing rhythm between caregiver and patient modes.

**Key Characteristics:**
- Near-white blue-tinted canvas (`{colors.canvas}` — #F8FBFF) with deep navy ink text (`{colors.ink}` — #0D1B2A). Clean without being sterile.
- Soft sky-blue primary CTA (`{colors.primary}` — #2D7DD2). Easy on the eyes, high enough contrast for WCAG AA compliance.
- Slab-serif display headlines via Copernicus / Tiempos Headline at weight 400 with negative letter-spacing. Pairs with humanist sans body for a warm, personal voice.
- Midnight-blue product surfaces (`{colors.surface-dark}` — #0A1628) for patient-facing screens — maximum contrast for low-vision accessibility.
- Light blue feature cards (`{colors.surface-card}` — #E2EFFA) — slightly deeper than canvas, used for memory cards and content groupings.
- Border radius is hierarchical: `{rounded.md}` (8px) for buttons + inputs, `{rounded.lg}` (12px) for content + memory cards, `{rounded.xl}` (16px) for the hero illustration container, `{rounded.pill}` for badges.
